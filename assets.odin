...
get_cubemap :: inline proc(name: string) -> Texture {
    return get_asset(Cubemap_Asset, name).texture;
}
...
catalog_load_cubemap :: proc(data: []byte, ctx: Asset_Load_Context) -> (^Cubemap_Asset, Asset_Load_Result, bool) {
    Cubemap_Spec :: struct {
        front, back, left, right, top, bottom: string,
    };

    containing_folder, dirok := basic.get_file_directory(ctx.tracked_file.path);
    assert(dirok);

    spec := wbml.deserialize(Cubemap_Spec, data);
    front_data,  ok1 := os.read_entire_file(tprint(containing_folder, spec.front));  assert(ok1); defer delete(front_data);  front,  w1, h1, pixel_format1, gpu_format1 := decode_png_data(front_data);
    bottom_data, ok2 := os.read_entire_file(tprint(containing_folder, spec.bottom)); assert(ok2); defer delete(bottom_data); bottom, w2, h2, pixel_format2, gpu_format2 := decode_png_data(bottom_data);
    top_data,    ok3 := os.read_entire_file(tprint(containing_folder, spec.top));    assert(ok3); defer delete(top_data);    top,    w3, h3, pixel_format3, gpu_format3 := decode_png_data(top_data);
    left_data,   ok4 := os.read_entire_file(tprint(containing_folder, spec.left));   assert(ok4); defer delete(left_data);   left,   w4, h4, pixel_format4, gpu_format4 := decode_png_data(left_data);
    right_data,  ok5 := os.read_entire_file(tprint(containing_folder, spec.right));  assert(ok5); defer delete(right_data);  right,  w5, h5, pixel_format5, gpu_format5 := decode_png_data(right_data);
    back_data,   ok6 := os.read_entire_file(tprint(containing_folder, spec.back));   assert(ok6); defer delete(back_data);   back,   w6, h6, pixel_format6, gpu_format6 := decode_png_data(back_data);

    assert(w1 == w2 && w2 == w3 && w3 == w4 && w4 == w5 && w5 == w6);
    assert(h1 == h2 && h2 == h3 && h3 == h4 && h4 == h5 && h5 == h6);
    assert(pixel_format1 == pixel_format2 && pixel_format2 == pixel_format3 && pixel_format3 == pixel_format4 && pixel_format4 == pixel_format5 && pixel_format5 == pixel_format6);
    assert(gpu_format1 == gpu_format2 && gpu_format2 == gpu_format3 && gpu_format3 == gpu_format4 && gpu_format4 == gpu_format5 && gpu_format5 == gpu_format6);

    skybox := create_cubemap();
    set_cubemap_textures(&skybox, cast(int)w1, cast(int)h1, right, left, top, bottom, front, back, gpu_format1, pixel_format1, .Unsigned_Byte);
    return new_clone(Cubemap_Asset{skybox}), .Ok, true;
}
