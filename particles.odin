...
init_particles :: proc() {
    ...
    gpu.set_vertex_format_ti(type_info_of(Particle_Vertex));
...
update_particle_emitter :: proc(using emitter: ^Particle_Emitter, delta_time: f32) {
    ...
    for _, i in particles {
        ...
        if !p.dead {
            ...
            p.colour = types.color_lerp(initial_colour, final_colour, complete_percent);
...
render_particle_emitter :: proc(using emitter: ^Particle_Emitter, projection, view: Mat4) {
    ...
    gpu.draw_arrays_instanced(.Triangles, 0, 6, active_particles);
    gpu.disable(.Blend);
