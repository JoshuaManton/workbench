...
ui_draw_colored_quad_current :: inline proc(color: Colorf) {
	...
	im_quad(.Pixel, wb_catalog.shaders["default"], min, max, color, {});
}
...
ui_draw_sprite_current :: proc(sprite: Sprite, loc := #caller_location) {
	...
	min := Vec2{cast(f32)rect.x1, cast(f32)rect.y1};
	max := Vec2{cast(f32)rect.x2, cast(f32)rect.y2};
	im_sprite(.Pixel, wb_catalog.shaders["default"], min, max, sprite);
}
...
ui_text_data :: proc(str: string, using data: ^Text_Data, loc := #caller_location) {
	...
	if shadow != 0 {
		im_text(.Unit, font, str, position+Vec2{cast(f32)shadow/platform.current_window_width, cast(f32)-shadow/platform.current_window_width}, shadow_color, height, current_render_layer); // todo(josh): @TextRenderOrder: proper render order on text
	}

	im_text(.Unit, font, str, position, color, height, current_render_layer); // todo(josh): @TextRenderOrder: proper render order on text
}
ui_text_args :: proc(font: Font, str: string, size: f32, color: Colorf, x1 := cast(f32)0, y1 := cast(f32)0, x2 := cast(f32)1, y2 := cast(f32)1, top := 0, right := 0, bottom := 0, left := 0, loc := #caller_location) {
	...
	im_text(.Unit, font, str, position, color, height * size, current_render_layer); // todo(josh): @TextRenderOrder: proper render order on text
}
