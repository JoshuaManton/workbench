...
ui_draw_colored_quad_current :: inline proc(color: Colorf) {
	...
	im_quad(rendermode_pixel, shader_rgba, to_vec3(min), to_vec3(max), color);
}
...
ui_draw_sprite_current :: proc(sprite: Sprite, loc := #caller_location) {
	...
	min := Vec2{cast(f32)rect.x1, cast(f32)rect.y1};
	max := Vec2{cast(f32)rect.x2, cast(f32)rect.y2};
	im_quad(rendermode_pixel, shader_texture, to_vec3(min), to_vec3(max), sprite);
}
...
ui_text_data :: proc(str: string, using data: ^Text_Data, loc := #caller_location) {
	...
	if shadow != 0 {
		im_text(rendermode_unit, font, str, position+Vec2{cast(f32)shadow/current_window_width, cast(f32)-shadow/current_window_width}, shadow_color, height, current_render_layer); // todo(josh): @TextRenderOrder: proper render order on text
	}
