...
main_render :: proc(dt: f32) {
    ...
    // draw ground
    wb.submit_draw_command(wb.create_draw_command(cube_model, wb.get_shader("lit"), math.Vec3{0, -2, 0}, {10, 1, 10}, math.Quat{0, 0, 0, 1}, types.Colorf{0.035, 1, 0, 1}, wb.Material{1, 1, 0.1}));

    // draw rotating cube
    wb.submit_draw_command(wb.create_draw_command(cube_model, wb.get_shader("lit"), math.Vec3{}, {1, 1, 1}, cube_rotation, types.Colorf{1, 0.8, 0, 1}, wb.Material{1, 1, 0.1}, {}));

    // draw light
    light_position := math.Vec3{math.sin(wb.time_since_startup) * 3, 2, math.cos(wb.time_since_startup) * 3};
    light_color := types.Colorf{1, 0, 1, 1};
    wb.push_point_light(light_position, light_color, 100);
    wb.submit_draw_command(wb.create_draw_command(cube_model, wb.get_shader("lit"), light_position, {.5, .5, .5}, math.Quat{0, 0, 0, 1}, light_color, wb.Material{1, 1, 0.1}, {}));
}
